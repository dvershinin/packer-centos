...
build:
	...
	@echo building $(TPL)
	$(foreach builder,$(builders), PACKER_LOG="yes" PACKER_CONFIG=$(CURDIR)/.packerconfig PACKER_LOG_PATH=$(CURDIR)/logs/$(TPL).log packer -machine-readable build -var "PROVISIONER=$(PROVISIONER)" -only=$(builder) $(TPL).json;)
